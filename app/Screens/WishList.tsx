  ...
  useEffect(() => {
    const loadWishlists = async () => {
      try {
        ...
        if (savedLists && savedItems) {
          const parsedLists = JSON.parse(savedLists);
          const parsedItems = JSON.parse(savedItems);
          
          // Create wishlist data from saved information
          const updatedWishlists = parsedLists.map((list: any) => {
            ...
            return {
              ...
              image: listItems.length > 0 ? { uri: listItems[0].image } : require("../../assets/images/burger.png"),
              items: listItems
            };
          });
          
          // Combine with default lists if they don't exist in saved lists
          const mergedData = [
            ...wishlistData.filter(item => !updatedWishlists.some(w => w.title === item.title)),
            ...updatedWishlists
          ];
    ...
    loadWishlists();
  }, [fontsLoaded]); // Load after fonts are loaded
